...
VIOLATION_DIR = r"D:\VITCS_FINAL-20250814T172227Z-1-001\new one\violation_integration\static\violations"
...
@app.route("/")
def index():
    process_violation_folder()  # 👈 AUTO OCR

    with sqlite3.connect(DB_NAME) as c:
        data = c.execute(
            "SELECT * FROM vehicle_data ORDER BY id DESC"
        ).fetchall()
